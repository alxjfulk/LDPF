function Lyme_Disease_Impulse_Intensity_FEM2D_Scenario2(jmax, kmax)
...
while check == 1
      ...
      Im(n,:) = U(:,9)';
      T(n) = t;
%for creating the gif file. Extract the individual images at the times
%mentioned in the paper to get the figures for scenario 2
      figure(1)
      clf
...
disp('Infectious Nymphs in Left Burned Area')
disp(sum(In(NN-1,0 <= X(:,1) & X(:,1) <= 10 & 20 <= X(:,2) & X(:,2) <= 30)))
disp('Infectious Nymphs in Right Burned Area')
disp(sum(In(NN-1,20 <= X(:,1) & X(:,1) <= 30 & 0 <= X(:,2) & X(:,2) <= 10)))
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
