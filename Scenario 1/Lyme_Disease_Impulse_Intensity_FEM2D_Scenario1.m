function Lyme_Disease_Impulse_Intensity_FEM2D_Scenario1(jmax, kmax)
...
for i = 1:Pulses  
    ...
    dt = 1e-5;
    dt0 = dt;
    %x1loc1 and x1loc2 define the x region where the burn is performed.
    %y1loc1 and y1loc2 define the y region where the burn is performed.
    %if you wish to have mutliple burns simultaneously, change x2 and y2,
    %otherwise a single burn is performed.
    x1loc1 = 10;
    x1loc2 = 20;
    y1loc1 = 10;
    y1loc2 = 20;
...
while check == 1
      ...
      Im(n,:) = U(:,9)';
      T(n) = t;
%this will save an image at each point of integration and create a gif file
%when finisehd executing. You'll need to use a program to extract all of
%the individual images at teh specific time points mentioned in the paper
%to get the figures for this scenario
      figure(1)
      clf
...
disp(sum(In(NN-1,10 <= X(:,1) & X(:,1) <= 20 & 10 <= X(:,2) & X(:,2) <= 20)))
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
